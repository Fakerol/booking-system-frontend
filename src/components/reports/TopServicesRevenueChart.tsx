...
export default function TopServicesRevenueChart() {
  ...
  const topServices = useMemo(() => {
    ...
    bookingData
      ...
      .forEach((booking) => {
        ...
        serviceRevenue[booking.service_name] = (serviceRevenue[booking.service_name] || 0) + price;
      });
    ...
    return sortedServices.map(([name, revenue]) => ({ name, revenue }));
  ...
  const options: ApexOptions = {
    ...
    xaxis: {
      categories: topServices.map((s) => s.name),
      axisBorder: {
        show: false,
      },
      axisTicks: {
