...
export default function BookingVolumeTrendChart() {
  const bookingData = useMemo(() => generateDummyBookings(), []);

  // Calculate booking volume trend (last 7 days)
  const bookingTrend = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayBookings = bookingData.filter((b) => b.date === dateStr);
  ...
  const options: ApexOptions = {
    ...
    xaxis: {
      categories: bookingTrend.map((d) => d.date),
      axisBorder: {
        show: false,
      },
      axisTicks: {
  ...
  const series = [
    {
      name: "Bookings",
      data: bookingTrend.map((d) => d.count),
    },
  ];
