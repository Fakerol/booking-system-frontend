...
// Generate dummy bookings data (same as in BookingTable)
...
export default function TopStaffBookingsChart() {
  // Get booking data
  const bookingData = useMemo(() => generateDummyBookings(), []);

  // Calculate bookings per staff and get top 3
  const topStaff = useMemo(() => {
    const staffCounts: Record<string, number> = {};
    
    bookingData.forEach((booking) => {
      staffCounts[booking.staff_name] = (staffCounts[booking.staff_name] || 0) + 1;
    });
    ...
    return sortedStaff.map(([name, count]) => ({ name, count }));
  }, [bookingData]);
  ...
  const options: ApexOptions = {
    ...
    xaxis: {
      categories: topStaff.map((staff) => staff.name),
      axisBorder: {
        show: false,
      },
      axisTicks: {
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
          Top 3 Staff with Most Bookings
        </h3>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Staff members with the highest number of bookings
        </p>
      </div>
